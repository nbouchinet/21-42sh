...
/*   Updated: 2017/10/10 13:08:19 by zadrien          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		exec_pro(t_process **lst, t_env **env, t_job **j)
{
	...
	if (!(tmp->pid = fork()))
		exec_fork(&tmp, n_env, (*j)->pgid, -1, 1);
	else
	{
...
int		exec_pipe_job(t_process **lst, char **env, int r, t_job **job)
{
	...
	if (pipe(p) == 0)
	{
		if ((tmp->pid = fork()) == 0)
		{
			...
			exec_fork(&tmp, env, (*job)->pgid, r, 1);
		}
		else
		{
...
int		exec_pipe_bg(t_process **pro, char **env, int r, t_job **job)
{
	...
	if (pipe(p) == 0)
	{
		if (!(tmp->pid = fork()))
		{
			close(p[0]);
			tmp->next != NULL ? dup2(p[1], STDOUT_FILENO) : 0;
			exec_fork(&tmp, env, (*job)->pgid, r, 0);
		}
		else
		{
...
int		exec_pro_bg(t_process **pro, t_env **env, t_job **job)
{
	...
	if (!(p->pid = fork()))
		exec_fork(&p, n_env, (*job)->pgid, -1, 0);
	else
	{
		fd_printf(2, "[%d] %d\n", (*job)->num, p->pid);
		(*job)->pgid = p->pid;
