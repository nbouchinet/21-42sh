...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/03/10 09:39:52 by khabbar           #+#    #+#             */
/*   Updated: 2017/03/10 09:39:57 by khabbar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../inc/libftprintf.h"

static void		ft_flags_x(t_b *b, char *str, char *rec, unsigned long int nbr)
{
	t_flags				flags;
	...
	if (ft_strchr(str, 'X'))
		ft_maj(&rec);
	ft_order_x(b, flags, str, rec);
}

static void		ft_newstr_x(t_b *b, char *str, va_list ap)
{
	char				*rec;
	...
	ft_itoa_base_ulong_nm(&rec, nbr, 16);
	ft_flags_x(b, str, rec, nbr);
}
...
void			ft_conv_x(va_list ap, t_b *b, char *format)
{
	...
	i = -1;
	b->error = 0;
	while (format[++i] != 'x' && format[i] != 'X' && format[i] != '\0')
		str[i] = format[i];
	str[i] = format[i];
	str[i + 1] = '\0';
	ft_newstr_x(b, str, ap);
}
